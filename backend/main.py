    ...
    try:
        ...
        if "application/json" in content_type:
            ...
            if isinstance(payload, dict):
                ...
                lang = str(payload.get("language") or "English")
                recipient_name = str(payload.get("recipient") or "Police Station")
        elif "application/x-www-form-urlencoded" in content_type or "multipart/form-data" in content_type:
            form = await request.form()
            story_text = str(form.get("story") or "").strip()
            lang = str(form.get("language") or "English")
            recipient_name = str(form.get("recipient") or "Police Station")

        if not story_text:
            raise HTTPException(
                status_code=422,
                detail="Missing complaint story. Send JSON {story, language, recipient} or form fields.",
            )
            ...
            messages=[
                {
                    ...
                    "content": f"You are an expert Indian legal assistant.\n"
                               f"Translate and format the following casual story into a formal, structured official complaint addressed to the {recipient_name} in {lang}.\n"
...
# ---------------------------------------------------------------------------
# SMS State Machine (separate from WhatsApp to avoid cross-contamination)
# ---------------------------------------------------------------------------
sms_recommend_state: dict[str, dict] = {}
...
async def sms_webhook(
    ...
    """
    Webhook for Twilio SMS integration.
    Handles plain-text SMS messages — no media/image/PDF support (that's MMS).
    Supports: RAG Q&A, scheme recommendations, scam detection, complaint drafting.
    """
    try:
        print(f"--- Received SMS from {From} ---")
        ...
        if not user_message:
            twiml.message(
                "Namaste! I am Saarthi. Send me any question about Indian government schemes, "
                "your rights, or agriculture and I will help you in your language!"
            )
            return Response(content=str(twiml), media_type="application/xml")

        # ── Recommendation State Machine ───────────────────────────────────────
        if From in sms_recommend_state:
            state = sms_recommend_state[From]
            step = state.get("step")
            lang = state.get("language", "English")

            if step == "occupation":
                occ_key = user_message.lower()
                occupation = OCCUPATION_MAP.get(occ_key) or OCCUPATION_MAP.get(user_message, "other")
                if occupation == "other":
                    for k, v in OCCUPATION_MAP.items():
                        if k in occ_key:
                            occupation = v
                ...
                del sms_recommend_state[From]
                profile = state["profile"]
                schemes = recommend_schemes(profile, language=lang, llm=rag_engine.llm, vector_db=rag_engine.vector_db)
                answer = format_whatsapp_recommendation(schemes, lang)
                # Strip bold markdown (*text*) since SMS doesn't render it
        ...
        if is_recommendation_request(user_message):
            detected_lang = rag_engine._detect_language(user_message)
            sms_recommend_state[From] = {
                "step": "occupation",
                "language": detected_lang,
                "profile": {"state": "All India"},
            }
            question = WA_QUESTIONS["occupation"].get(detected_lang, WA_QUESTIONS["occupation"]["English"])
            twiml.message(strip_markdown(question))
            return Response(content=str(twiml), media_type="application/xml")

        # Complaint / FIR drafting
        if (
            user_message.lower().startswith("draft:")
            or user_message.lower().startswith("draft ")
            or "complaint" in user_message.lower()
            or "शिकायत" in user_message.lower()
        ):
            intent_check = client.chat.completions.create(
                model="gpt-5.4-nano",
                messages=[{
                    "role": "user",
                    "content": "Return 'TRUE' if the user is asking to write, draft, or format a complaint/FIR. Otherwise return 'FALSE'.\n\nUser message: " + user_message,
                }],
            ).choices[0].message.content.strip()

            if "TRUE" in intent_check:
                print("--- SMS: Complaint Drafting request detected ---")
                detected_lang = rag_engine._detect_language(user_message)
                try:
                    draft_resp = client.chat.completions.create(
                        model="gpt-5.4-nano",
                        messages=[{
                            "role": "user",
                            "content": (
                                f"You are an expert Indian legal assistant.\n"
                                f"Translate and format the following casual story into a formal, structured official complaint "
                                f"addressed to the Police Station or relevant authority in {detected_lang}.\n"
                                f"Maintain a respectful, formal, and objective legal tone.\n"
                                f"Include standard placeholders like [Date], [Your Name], [Your Address], and [Signature].\n"
                                f"Keep it concise — SMS has a character limit.\n\n"
                                f"Story:\n{user_message}"
                            ),
                        }],
                    ...
                    draft = strip_markdown(draft_resp.choices[0].message.content)
                    # SMS: send in two parts if needed, but keep it under 1600 chars
                    sms_body = f"Saarthi Legal Draft\n\n{draft}"
                    twiml.message(sms_body[:1600])
                    return Response(content=str(twiml), media_type="application/xml")
                except Exception as e:
                    print(f"SMS Complaint Error: {e}")

        # Scam detection
        scam_triggers = ["fake", "real", "scam", "fraud", "सही है", "फेक", "धोखा"]
        if len(user_message) > 150 or any(t in user_message.lower() for t in scam_triggers):
            print("--- SMS: Potential Scam/Forward detected ---")
            verdict = verify_scheme_message(user_message, language="Auto", llm=rag_engine.llm, vector_db=rag_engine.vector_db)

            if verdict.get("verdict") in ["Fake", "Suspicious"]:
                icon = "WARNING" if verdict["verdict"] == "Suspicious" else "ALERT"
                response_text = f"SAARTHI {icon}: {verdict['verdict'].upper()}\n\n"
                response_text += f"Reason: {verdict.get('reasoning', '')}\n"
                if verdict.get("official_details"):
                    response_text += f"\nOfficial Info: {verdict['official_details']}"
                twiml.message(response_text[:1600])
                return Response(content=str(twiml), media_type="application/xml")
        ...
        traceback.print_exc()
        twiml = MessagingResponse()
        twiml.message("I am sorry, I am having trouble right now. Please try again later!")
        return Response(content=str(twiml), media_type="application/xml")
